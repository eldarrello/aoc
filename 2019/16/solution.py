...
s1 = list(map(int, s))
s2 = list(map(int, (s * 10000)[int(s[:7]):][::-1]))
for t in range(100):
    temp = []
    for p in range(1, len(s1) + 1):
        pattern = [0] * p + [1] * p + [0] * p + [-1] * p
        ack = 0
        for i in range(len(s1)):
            ack += s1[i] * pattern[(i + 1) % len(pattern)]
        temp.append(abs(ack) % 10)
    s1 = temp
    ack = 0
    for i in range(len(s2)):
        ack += s2[i]
        s2[i] = ack % 10
print('Part 1 message: ', *s1[0:8], '\nPart 2 message: ', *s2[::-1][:8], sep = '')
